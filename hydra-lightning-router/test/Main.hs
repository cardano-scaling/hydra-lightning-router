...
payToPlutusScript ::
  ...
  C.PlutusScript lang ->
  m C.TxIn
payToPlutusScript script = inBabbage @era $ do
  let tx = execBuildTx $ payToScriptDatumHash Defaults.networkId (plutusScript script) () C.NoStakeAddress (C.lovelaceToValue 10_000_000)
  ...
  testGroup
    ...
    [ testGroup
        ...
        [ testCase "spending a public key output to alwaysSucceeds script" (mockchainSucceeds $ failOnError $ payToPlutusScript alwaysSucceedsScript),
          ...
          testCase "spending a script output from htlc script" (mockchainSucceeds $ failOnError (payToPlutusScript htlcValidatorScript >>= spendFromPlutusScript htlcValidatorScript))
        ]
    ]
